...
// 将 sitetopic 行拼成与 topic_monitor_list.txt 完全一致的行格式
if (!function_exists('build_topic_line')) {
    function build_topic_line($row) {
        return implode('|', [
            (string)$row['ctx_id'],
            (string)$row['keyword'],
            (string)$row['status'],
...
if (!empty($_GET['t'])) {
        ...
        if ($q == "all") {
            ...
            $rows = Database::fetchAll(
                'SELECT * FROM "sitetopic" WHERE "status" = :status AND "keyword" = :keyword'
                . ' AND (json_extract("json", \'$.lasttask\') IS NULL OR json_extract("json", \'$.lasttask\') != :today)'
                . ' ORDER BY RANDOM() LIMIT 1',
                ['status' => 'enable', 'keyword' => $q, 'today' => $time]
            );
        }

        $response = array();
        foreach ($rows as $row) {
            $response[] = build_topic_line($row);
        }
        ...
        if (!empty($response)) {
            ...
            foreach ($response as $line) {
                ...
                if (!empty($keyword)) {
                    ...
                    // 本地直写：标记 lasttask / status（等价于远端 topicedit.php 回写，
                    // 但避免网络往返、鉴权依赖与静默失败）
