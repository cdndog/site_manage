...
// Helper function to get completion rate color class
function getCompletionColorClass($rate) {
    if ($rate >= 80) {
        return 'text-green-600';  // 高完成率 - 绿色
    } elseif ($rate >= 50) {
        return 'text-yellow-600'; // 中完成率 - 黄色
    } else {
        return 'text-red-600';    // 低完成率 - 红色
    }
}

// Helper function to get completion rate progress bar color
function getCompletionColor($rate) {
    if ($rate >= 80) {
        return 'bg-green-500';    // 高完成率 - 绿色
    } elseif ($rate >= 50) {
        return 'bg-yellow-500';   // 中完成率 - 黄色
    } else {
        return 'bg-red-500';      // 低完成率 - 红色
    }
}
                ...
                <div class="overflow-x-auto">
                    <table id="table-date" class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <?php
                                // 完成率 = 已完成 / 总数 * 100（只计算真正完成的 aidone）
                                $completionRate = $stats['total'] > 0
                                    ? round($stats['aidone'] / $stats['total'] * 100, 2)
                                    : 0;
                                $colorClass = getCompletionColorClass($completionRate);
                                $progressColor = getCompletionColor($completionRate);
                            ?>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <div class="flex items-center">
                                        <div class="flex-1 w-32 bg-gray-200 rounded-full h-2.5 mr-2">
                                            <div class="<?php echo $progressColor; ?> h-2.5 rounded-full" style="width: <?php echo $completionRate; ?>%"></div>
                                        </div>
                                        <span class="text-xs font-medium <?php echo $colorClass; ?>"><?php echo $completionRate; ?>%</span>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
...
<script>
    ...
    function sortTable(tableId, n, type) {
        ...
        rowArray.sort(function(rowA, rowB) {
            ...
            if (type === 'number') {
                // 数字排序 - 提取纯数字进行比较
                var numA = parseFloat(valueA.replace(/[^0-9.-]/g, ''));
                var numB = parseFloat(valueB.replace(/[^0-9.-]/g, ''));
                ...
                // 日期排序 - 支持 YYYYMMDD 格式
                var dateA = parseInt(valueA.replace(/[^0-9]/g, '').substring(0, 8));
                var dateB = parseInt(valueB.replace(/[^0-9]/g, '').substring(0, 8));
                ...
                // 文字排序 - 使用中文 localeCompare
                compareResult = valueA.localeCompare(valueB, 'zh-CN', {
                    numeric: true,
                    sensitivity: 'base'
                });
            }
