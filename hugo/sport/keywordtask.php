...
if (empty($remoteServerURL)) {
    $remoteServerURL = "https://sport.betmanjapan.com";
}
    ...
    if (!empty($_GET['t'])) {
        ...
        if ($q == "all") {
            ...
            usort($lines, function($a, $b) {
                $aParts = explode('|', $a);
                $bParts = explode('|', $b);
                $aData = json_decode(end($aParts), true);
                $bData = json_decode(end($bParts), true);

                // Set default lasttask if not set or empty
                $aLasttask = !empty($aData['lasttask']) ? $aData['lasttask'] : '99999999';
                $bLasttask = !empty($bData['lasttask']) ? $bData['lasttask'] : '99999999';

                // return strcmp($bLasttask, $aLasttask); // Reverse order
                return strcmp($aLasttask, $bLasttask); // ascending order
            });
            ...
            foreach ($lines as $key => $line) {
                ...
                if ( $alldata['lasttask'] !== $time && !empty($parts[0]) && $parts[2] == "enable" ) {
        ...
        if (!empty($response)) {
            ...
            foreach ($lines as $line) {
                ...
                $status = $keywordData['status'] ?? "enable";
                $pubdir = $keywordData['pubdir'] ?? "";

                if (!empty($keyword)) {
                    // $url = 'http://43.153.41.108/hugo/keywordops.php';
                    $url = $remoteServerURL.'/hugo/keywordops.php';
                    $data = [
